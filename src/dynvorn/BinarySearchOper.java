...
public class BinarySearchOper implements TestBedOperation, Globals {
  /*! .enum  .private 3600  sampleres printhist persist allevents
  */

    private static final int SAMPLERES        = 3600;//!
    private static final int PRINTHIST        = 3601;//!
    private static final int PERSIST          = 3602;//!
    private static final int ALLEVENTS        = 3603;//!
/* !*/

  public void addControls() {
    C.sOpenTab("Voronoi search");
    {
      C.sStaticText("Determines topologically distinct Voronoi diagrams by"
          + " performing a binary search to find the points in time when the"
          + " topology changes.  Maintains a list of distinct string "
          + "representations of the diagrams, sorted by time they appeared.");
  ...
  public static BinarySearchOper singleton = new BinarySearchOper();

  private BinarySearchOper() {
  }
  private void updatePointSet(DArray vs) {

    {
      ...
      String rep = sb.toString(); //vs.toString(true);
      ...
      if (history == null || lastPointSet == null || !lastPointSet.equals(rep)
        ...
        if (history != null)
          history.stopThread();
        history = new VornHistory(vs, 1200, lastSampleRes);
      }
    }
  }
  ...
  public void paintView() {
    ...
    if (history != null) {
      ...
      //   V.draw("T=" + Tools.f(Main.dispTime()).trim(), 0, 100, TX_CLAMP);
      V.pop(2);
